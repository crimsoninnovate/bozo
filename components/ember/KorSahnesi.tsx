...
import { KorKivilcimi } from './KorKivilcimi'
...
export function KorSahnesi({ varyant }: Props) {
  ...
  return (
    <div className={`${stil.kap} ${anaMi ? stil.ana : stil.ic}`} aria-hidden="true">
      ...
      {/* Katman sırası ocağın kendi sırası: kor, üstünde yükselen kıvılcım,
          sonra imleç koru (Ana:31) ve en üstte duman. */}
      {anaMi && <KorKivilcimi />}
      {anaMi && <ImlecKoru />}
      {anaMi ? (
        <>
      ...
      {/* Sahnenin alt ucu ekranın dibinde sert bir çizgiyle bitiyordu; bu katman
          onu zemine söndürür. UYGULAMA-NOTLARI 1.3 zorunlu tutuyor. */}
      <span className={stil.dip} />
    </div>
  )
}
